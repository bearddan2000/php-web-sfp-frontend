...
function start-up(){
    ...
    if [[ -e "web.tar.gz" && ! -d "web" ]]; then

      echo "$info_base extracting web dir" >> $logfile

      sudo tar -zxvf web.tar.gz web

      sudo chmod -R +x web/* && cd web
...
function tear-down(){
    ...
    if [[ -d "web" ]]; then
      ...
      echo "$info_base removing web folder" >> $logfile
